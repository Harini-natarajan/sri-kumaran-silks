...
const About = () => {
    ...
    return (
        <div className="bg-amber-100 pt-24 md:pt-32">
            {/* Simple Hero Section */}
                ...
                <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-8">
                    {stats.map((stat, index) => (
                        <div key={index} className="text-center group">
                            ...
                            <div className="text-xs text-gray-500 uppercase tracking-widest font-medium mt-1">{stat.label}</div>
                        </div>
                    ))}
                </div>
            </section>
            ...
            <section className="py-20 bg-amber-50/30">
                <div className="max-w-7xl mx-auto px-4">
                    <div className="grid lg:grid-cols-2 gap-16 items-center mb-20">
                        <div>
                            <span className="text-amber-600 font-bold text-xs tracking-widest uppercase mb-4 block">The Beginning</span>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {values.map((value, index) => (
                                <div key={index} className="bg-white p-6 rounded-2xl shadow-sm border border-amber-100/50 hover:shadow-md transition-all group">
                                    <div className="w-12 h-12 mb-4 bg-amber-50 rounded-xl flex items-center justify-center text-amber-700 group-hover:bg-[#9A3412] group-hover:text-white transition-colors duration-300">
                                        {React.cloneElement(value.icon, { size: 24 })}
                                    </div>
                                    <h3 className="text-lg font-serif font-bold text-gray-900 mb-2">{value.title}</h3>
                    ...
                    {/* Final Promise */}
                    <div className="bg-white rounded-3xl p-8 md:p-12 shadow-xl border border-amber-100 relative overflow-hidden text-center max-w-4xl mx-auto">
                        <div className="relative z-10">
                            <h3 className="text-2xl font-serif font-bold text-gray-900 mb-4 text-center">Our Commitment</h3>
                            <div className="flex flex-wrap justify-center gap-6 md:gap-12">
                                <div className="flex items-center gap-2">
                                    <Shield className="text-amber-600" size={20} />
                                    <span className="text-sm font-medium text-gray-700">Silk Mark Certified</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <CheckCircle className="text-amber-600" size={20} />
                                    <span className="text-sm font-medium text-gray-700">100% Authentic Zari</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <Truck className="text-amber-600" size={20} />
                                    <span className="text-sm font-medium text-gray-700">Global Delivery</span>
                                </div>
                            </div>
                        </div>
